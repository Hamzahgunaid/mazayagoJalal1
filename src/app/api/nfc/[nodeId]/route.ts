...
export async function GET(req: Request, ctx: { params: Promise<{ nodeId: string }> }) {
  try {
    ...
    // اجلب slug الخدمة لهذه الـ Node
    ...
    // سجّل النقرة (تحليلات)
    await pool.query(
      `INSERT INTO public.nfc_taps(service_node_id) VALUES ($1)`,
      [nodeId]
    );

    // أنشئ جلسة NFC طويلة الأمد
    const sess = await pool.query(
      `SELECT token FROM public.create_nfc_session($1::uuid, NULL::uuid, $2::int, $3::int)`,
      [nodeId, 3650, 100000000]
    );
