...
const createDefaultTaskForm = () => ({
  title: '',
  description: '',
  kind: RIDDLE_TASK_KINDS[0]?.value ?? 'MCQ',
  options: ensureOptions([createEmptyOption(true), createEmptyOption(false), createEmptyOption(false)]),
});

export default function RiddleChallenges({ contestId }: { contestId: string }) {
  const t = useTranslations('OfferManage.riddle');
  ...
  function startEditTask(task: Task) {
    ...
    setTaskForm({
      ...
      kind: RIDDLE_TASK_KINDS.some((opt) => opt.value === task.kind) ? task.kind : 'MCQ',
      options,
    });
  }
  ...
  function handleKindChange(nextKind: string) {
    setTaskForm((prev) => ({
      ...prev,
      kind: nextKind,
      options: nextKind === 'MCQ' ? ensureOptions(prev.options) : ensureOptions(createDefaultTaskForm().options),
    }));
  }
  ...
  const getKindLabel = (value: string) => {
    const match = RIDDLE_TASK_KINDS.find((item) => item.value === value);
                        ...
                        <input
                          ...
                          onChange={() =>
                            setTaskForm((prev) => ({
                              ...
                              options: prev.options.map((opt) => ({
                                ...opt,
                                is_correct: opt.tempId === option.tempId,
                              })),
                            }))
                          }
                        />
                        {t('builder.options.correct')}
                      </label>
                      <button
                        type="button"
                    ...
                    setTaskForm((prev) => ({
                      ...
                      options:
                        prev.options.length >= MAX_OPTIONS
                          ? prev.options
                          : [...prev.options, createEmptyOption(false)],
                    }))
                  }
                ...
                {task.kind === 'MCQ' && task.options && task.options.length > 0 && (
                  <div className="mt-3 rounded-2xl border border-border bg-bg p-3">
                    ...
                    <ul className="space-y-1 text-xs text-muted">
                      {task.options.map((option) => (
                        <li key={option.id || option.position || option.label} className="flex items-center justify-between">
